...
<?php
$sql = "SELECT * FROM allorders ORDER BY date desc";
...
$orderid=$row['orderid'];
$stetus=$row['stetus'];
$deliveryboyname=$row['deliveredby'];

echo "<tr>";
                echo "<td><a href='oneorder.php?orderid=" . $row['orderid'] . "'>" . $row['orderid'] . "</a></td>";


                $usernameofuser = $row['mobileno'];
                $sqlasasas = "SELECT * FROM userid where mobileno = '$usernameofuser'";
                $resultaaa = $conn->query($sqlasasas);
                
                if ($resultaaa->num_rows > 0) {
                    // output data of each row
                    while($rowaaa = $resultaaa->fetch_assoc()) {
                echo "<td>" . $rowaaa["firstname"]. " <a href ='tel:" . $row["mobileno"]. "'>" . $row["mobileno"]. "</a></td>";
                    }
                }


                echo "<td><b>(" . $row['cityname'] . ")</b> " . $row['deliverytime'] . " at " . $row['deliveryaddress'] . "</td>";
                echo "<td>" . $row['total'] . "/-</td>";
                echo "<td>" . $row['deliverycharge'] . "/-</td>";
                
if($stetus == '' || $stetus == 'processing'){
  echo "<td>" . " <select name='forma' onchange='location = this.value;'>
  <option value='' selected>Stetus " . $stetus . " by " . $deliveryboyname . "</option>
  ...
  echo "<td>" . $stetus . " by " . $deliveryboyname . "</td>";
}
